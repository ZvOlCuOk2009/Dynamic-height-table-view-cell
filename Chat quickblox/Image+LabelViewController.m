...
#define kCellHeight 465.0

static NSString *const CellIdentifier = @"image+LabelCell";
...
- (void)viewDidLoad {
    ...
    self.dataSource = @[@"VaxVoIP SIP call recording SDK provides tools, software components and sample codes to build SIP REC protocol based call recording servers. This doesn’t come from the cell itself, instead I am just storing the selected state in an NSMutableDictionary keyed on the indexPath. Animate The Change So we know that returning a variable height in the heightForRowAtIndexPath method will result in the table sizing the cells accordingly, but how do we animate the change… \nWell simplicity itself… you simply use these two lines… What this results in is the UITableView re-evaluating its visible cells and setting their size accordingly but without reloading the data, and the best part?",
                        ...
                        @"Versatile approach to design and develop MS Windows OS based SIP REC ",
...
- (void)setUpCell:(TableViewCell *)cell atIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.titleLabel.preferredMaxLayoutWidth = height - 30;
    ...
    NSString *text = @"Ольга Полякова";
    cell.nameLabel.text = [text stringByReplacingOccurrencesOfString:@" " withString:@"\n"];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([self cellIsSelected:indexPath]) {
        heightCell = [self calculateHeightForConfiguredSizingCell:cell];
    }
